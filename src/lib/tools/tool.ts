 ...
 * Beautiful, consistent icon library with 1000+ SVG icons.
 * Used here to assign icons to each AI tool (globe for search, lightbulb for reasoning, etc.).
 * World use cases: Any web app needing clean, modern icons.
 */
import { GlobeIcon, Lightbulb, Zap, ChartLine } from "lucide-react";

/**
 * === UPLOADTHING SERVER ("uploadthing/server") ===
 * Server-side file upload utilities.
 * - UTApi: Server API for uploading, deleting, and managing files on UploadThing's CDN.
 * - UTFile: Creates file objects from buffers for upload (used for AI-generated images).
 * World use cases: Any app needing server-side file uploads (profile pics, documents, media).
 */
import { UTApi, UTFile } from "uploadthing/server";

/**
 * === LUKEED UUID ("@lukeed/uuid") ===
 * A faster, smaller alternative to the standard uuid package.
 * Used here to generate unique IDs for AI-generated image files.
 * World use cases: Same as uuid — unique IDs for anything.
 */
import { v4 as uuid } from "@lukeed/uuid";

/**
 * === GOOGLE AI PROVIDER ("@ai-sdk/google") ===
 * Used here specifically for:
 * 1. Image generation via Gemini 2.5 Flash Image model
 * 2. Market research with Google Search grounding (google.tools.googleSearch)
 */
import { google } from "@ai-sdk/google";

/**
 * === VERCEL AI SDK ("ai") — Tool & text generation functions ===
 * - generateText: One-shot text generation (used for market research data + summaries).
 * - generateObject: Structured JSON output from AI — used to generate chart configurations
 *   that follow a strict Zod schema (bar/line chart with labels, data, colors).
 * - tool: Defines a callable AI tool with typed input/output schemas.
 *   The AI model can call these tools during a conversation (e.g., "search the web for X").
 * World use cases: AI agents, function calling, structured data extraction.
 */
 ...
 * Used here to define the exact input/output shapes for each AI tool.
 * The AI model sees these schemas and knows what parameters to pass and what to expect back.
 * World use cases: API validation, form validation, config parsing, AI tool definitions.
 */
import { z } from "zod";
...
/**
 * === EXA CLIENT (local wrapper for "exa-js") ===
 * Exa is an AI-powered web search engine.
 * getExaClient creates a client that calls Exa's API to search the web
 * and return content summaries — powers the "Search Web" tool.
 * World use cases: AI research tools, RAG pipelines, real-time info retrieval.
 */
