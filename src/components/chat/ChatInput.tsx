 ...
 * Performant form management library — manages form state without re-rendering
 * the entire form on every keypress. Used here to manage the chat input form.
 * World use cases: Login forms, signup, settings, checkout, search — any form.
 */
import { useForm } from "react-hook-form";

/** === ZOD type inference — extracts TypeScript types from Zod schemas === */
import type { z } from "zod";

/**
 * === HOOKFORM RESOLVERS ("@hookform/resolvers") ===
 * Bridges validation libraries (Zod, Yup, etc.) with react-hook-form.
 * zodResolver: Validates form data against a Zod schema on submit.
 * World use cases: Any react-hook-form project needing schema-based validation.
 */
import { zodResolver } from "@hookform/resolvers/zod";

/**
 * === VERCEL AI SDK ("ai") — TypeScript types ===
 * - ChatRequestOptions: Options for customizing chat requests.
 * - CreateUIMessage: Type for constructing new messages to send.
 * - FileUIPart: Type for file attachment parts within messages.
 * - UIMessage: The main message type (id, role, parts array).
 */
import type {
  ChatRequestOptions,
  CreateUIMessage,
...
/** === Local UI components (built with Radix UI primitives + CVA) === */
...
/**
 * === LUCIDE REACT — Icons ===
 * ArrowUp: Send button icon | FileIcon/ImageIcon: File type indicators | X: Close/remove
 */
import { ArrowUp, FileIcon, ImageIcon, X } from "lucide-react";

/** === Local utility — cn() for conditional class merging === */
import { cn } from "@/lib/utils";

/** === Local components for model/tool selection dropdowns === */
import ModelDropDown from "./ModelDropDown";
import ToolDropDown from "./ToolDropDown";

/** === Local Zod schema defining the chat input form shape === */
import { chatInputSchema } from "@/schemas";

/** === Local model types and cookie persistence === */
import { DEFAULT_MODEL_ID, type ModelId } from "@/lib/model/model";
import { saveChatModelAsCookie } from "@/lib/model";

/** === Local tool types and cookie persistence === */
import { type Tool, TOOL_REGISTRY } from "@/lib/tools/tool";
import { saveToolAsCookie } from "@/lib/tools";

/** === Local file upload utilities (wraps UploadThing) === */
import { deletedAttachedFile, uploadTextFile } from "@/lib/upload";

/**
 * === SONNER ("sonner") ===
 * Toast notifications — used here for upload success/error feedback.
 */
 ...
 * useMutation: Calls Convex write operations (updateMessage, createAttachment).
 */
import { useMutation } from "convex/react";
import { api } from "@convex/_generated/api";
import { Doc } from "@convex/_generated/dataModel";

/** === Local hooks for mobile detection and usage rate limiting === */
import { useIsMobile } from "@/hooks/use-mobile";
import { useUsageStatus } from "@/hooks/use-usage-status";

/**
 * === FRAMER MOTION ("framer-motion") ===
 * Used here for the rate-limit warning animation (fade-in + slide-up).
 */
